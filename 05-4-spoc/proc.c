...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    proc = alloc_proc();
    proc->pid = get_pid();
    cprintf("alloc_proc: pid = %d, state = %d\n", proc->pid, proc->state);
...
repeat:
    ...
    if (haskid) {
        ...
        cprintf("pid = %d, state %d -> state %d\n", current->pid, current->state, PROC_SLEEPING);
        current->state = PROC_SLEEPING;
        cprintf("pid = %d, wait_state %d -> wait_state %d\n", current->pid, current->wait_state, WT_CHILD);
...
do_exit(int error_code) {
	...
	cprintf(" do_exit: proc  parent %x, parent pid = %d\n", current->parent, current->parent->pid);
    cprintf("pid = %d, state %d -> state %d\n", current->pid, current->state, PROC_ZOMBIE);
...
proc_init(void) {
    ...
    set_proc_name(initproc3, "thread3");
    cprintf("proc_init:: Created kernel thread init_main--> pid: %d, name: %s, state = %d, wait_state = %d\n",initproc1->pid, initproc1->name, initproc1->state, initproc1->wait_state);
	cprintf("proc_init:: Created kernel thread init_main--> pid: %d, name: %s, state = %d, wait_state = %d\n",initproc2->pid, initproc2->name, initproc2->state, initproc2->wait_state);
    cprintf("proc_init:: Created kernel thread init_main--> pid: %d, name: %s, state = %d, wait_state = %d\n",initproc3->pid, initproc3->name, initproc3->state, initproc3->wait_state);
    assert(idleproc != NULL && idleproc->pid == 0);
}

static int thread3(void *arg){
    cprintf("thread3 begin running, pid = %d, name = %s, state = %d\n", current->pid, get_proc_name(current), current->state);
    cprintf("thread3 running...\n");
    return 0;
}
