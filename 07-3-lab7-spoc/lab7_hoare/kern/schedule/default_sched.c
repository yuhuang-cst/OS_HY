...
stride_enqueue(struct run_queue *rq, struct proc_struct *proc) {
      ...
      */
      ...
      //使用链表，与RR算法一致
      assert(list_empty(&(proc->run_link))); //run_link中应该仅有一项
      list_add_before(&(rq->run_list), &(proc->run_link)); //加入就绪队列
...
stride_dequeue(struct run_queue *rq, struct proc_struct *proc) {
      ...
      */
      ...
      //与RR算法的实现一致
      assert(!list_empty(&(proc->run_link)) && proc->rq == rq);//检查队列非空，且进程位于队列中
