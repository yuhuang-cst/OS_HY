...
setup_user_paging()
{
  ...
  int i;
  int *pg_tbl_user = pg_dir + 1024 * 17;
  pg_dir[0x4 << 6] = (int)pg_tbl_user | PTE_P | PTE_W | PTE_U;
  for (i = 0; i < 256; ++i)
    pg_tbl_user[i] = ((0x2000 + i) << 12) | PTE_P | PTE_W | PTE_U;
...
setup_kernel_paging()
{
  ...
  for(i = 0; i < 16; ++i)
    pg_dir[i + (0xc << 6)] = (int)pg_tbl[i] | PTE_P | PTE_W;

  for (i = 0; i < 16; ++i){
    for (j = 0; j < 1024; ++j){
      pg_tbl[i][j] = ((i * 1024 + j) << 12) | PTE_P | PTE_W;
    }
  }
  pg_dir[0] = pg_dir[0xc << 6];
}
