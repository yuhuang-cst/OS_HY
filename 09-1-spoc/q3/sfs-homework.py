...
class fs:
    ...
    # DONE
        ...
        pDnum = self.inodes[pInum].getAddr()
        if self.data[pDnum].getFreeEntries() == 0: #no room in parent directory
            return -1
    ...
    def createSoftLink(self, target, newfile, parent):
        ...
        if inum == -1:#no room in inode
            return -1

        dnum = self.dataAlloc()
        if dnum == -1:#no room in data
            self.inodeFree(inum)
            return -1

        tinum = self.nameToInum[target]
        self.data[dnum].setType('lf')
        self.data[dnum].addSoftLink(tinum)
        self.inodes[inum].setAll('lf', dnum, 1)

        self.inodes[pInum].incRefCnt()
        self.data[pDnum].addDirEntry(newfile, inum)
    ...
    # DONE
        ...
        if inum == -1:#no room in inode
            return -1
        if self.data[pDnum].dirEntryExists(newfile): #file name is not unique
            return -1
