...
if options.solve == True:
    ...
    if options.policy == 'SJF':
        ...
        t = 0.0
        for job in joblist:
            print "[ time   %d ] Run job %d for %3.2f secs ( DONE at %3.2f )" % (t, job[0], job[1], t+job[1])
            t += job[1]
        print '\nFinal statistics:'
        t     = 0.0
        count = 0
    ...
    if options.policy == 'MLFQ':
        ...
        runqueue = [{"quant" : quantum, "joblist" : runlist}]
        thetime  = 0.0
        while jobcount > 0:
            #print runqueue
            for priority in range(0, len(runqueue)):
                if runqueue[priority]["joblist"]:
                    job = runqueue[priority]["joblist"].pop(0)
                    jobnum = job[0]
                    runtime = float(job[1])
                    quant = runqueue[priority]["quant"]
                    if response[jobnum] == -1:
                        response[jobnum] = thetime
                    currwait = thetime - lastran[jobnum]
                    wait[jobnum] += currwait
                    if runtime > quant:
                        ...
                        ranfor = quant
                        print '  [ time %3d ] Run job %3d for %.2f secs' % (thetime, jobnum, ranfor)
                        if priority == len(runqueue) - 1:
                            runqueue.append({"quant" : quant * 2, "joblist" : []})
                        runqueue[priority + 1]["joblist"].append([jobnum, runtime])
                    else:#当runtime <= quant时，可能是因为程序执行完毕 或 程序进行IO而放弃CPU，此处仅考虑第一种情况
                        ranfor = runtime
                        print '  [ time %3d ] Run job %3d for %.2f secs ( DONE at %.2f )' % (thetime, jobnum, ranfor, thetime + ranfor)
                        turnaround[jobnum] = thetime + ranfor
                        jobcount -= 1
